import React from "react";
import SolutionPageLayout from "@/components/layout/SolutionPageLayout";
import {
 FileText,
 Settings,
 Layers,
 Box,
 BarChart,
 ShieldCheck,
 Globe,
 Rocket,
 Zap,
 Briefcase,
 CheckCircle2,
 Share2,
 Workflow,
 Wrench
} from "lucide-react";

// Data Structures
const strategyServices = [
 {
 title: "Assess & Define",
 icon: <Briefcase className="w-8 h-8 text-orange-600" />,
 items: ["Assess current development processes", "Define PLM strategy aligned with goals"],
 },
 {
 title: "Digital Roadmap",
 icon: <Map className="w-8 h-8 text-orange-600" />, // Using alias or standard Map icon
 items: ["Create digital transformation roadmap", "SAP PLM implementation planning"],
 },
 {
 title: "Implementation",
 icon: <Settings className="w-8 h-8 text-orange-600" />,
 items: ["Core PLM (Part, Doc, BOM Mgmt)", "Integration with MES & Supply Chain"],
 },
];

const integrationServices = [
 {
 title: "Core Implementation",
 desc: "SAP PLM core implementation including Part, Document, and BOM Management.",
 },
 {
 title: "System Integration",
 desc: "Integration with SAP S/4HANA, ECTR, Teamcenter, and CAD tools (NX, CATIA).",
 },
 {
 title: "Workflow Automation",
 desc: "Engineering Change Management and workflow automation across systems.",
 },
];

const collaborationAndBom = [
 {
 title: "Engineering Collaboration",
 icon: <Share2 className="w-6 h-6" />,
 desc: "SAP ECTR deployment for multi-CAD data management and visualization.",
 },
 {
 title: "BOM Management",
 icon: <Box className="w-6 h-6" />,
 desc: "Harmonization of eBOM, mBOM, and sBOM across departments.",
 },
 {
 title: "Variant Configuration",
 icon: <Layers className="w-6 h-6" />,
 desc: "Product modeling and variant configuration for complex products.",
 },
];

const complianceAndChange = [
 {
 title: "Document Management",
 icon: (
 <FileText className="w-12 h-12 mx-auto text-slate-400 group-hover:text-orange-200 mb-6 transition-colors" />
 ),
 desc: "Lifecycle version control, audit trails, and digital signatures.",
 },
 {
 title: "Regulatory Compliance",
 icon: (
 <ShieldCheck className="w-12 h-12 mx-auto text-slate-400 group-hover:text-orange-200 mb-6 transition-colors" />
 ),
 desc: "Management of REACH, RoHS, ISO, FDA, and other regulatory standards.",
 },
 {
 title: "Change Management",
 icon: (
 <Workflow className="w-12 h-12 mx-auto text-slate-400 group-hover:text-orange-200 mb-6 transition-colors" />
 ),
 desc: "End-to-end change management (ECR/ECO) and impact analysis.",
 },
];

const analyticsAndSupport = [
 "Product performance insights & KPI dashboards",
 "Compliance and lifecycle reporting",
 "Application management services (AMS)",
 "PLM system upgrades and optimization",
 "User training and change management"
];





import { Map } from "lucide-react";


const trustedPartnerFeatures = [
  { icon: <Briefcase className="w-8 h-8" />, label: "Industry Expertise" },
  { icon: <ShieldCheck className="w-8 h-8" />, label: "Certified Professionals" },
  { icon: <Globe className="w-8 h-8" />, label: "Global Delivery Model" },
  { icon: <Rocket className="w-8 h-8" />, label: "Rapid Deployment" },
];
const endToEndServices = ["Cloud & On-Prem Expertise", "Innovation-Driven", "Change Management & Training", "Value Realization & Continuous Improvement", "License Advisory & Optimization", "Security & Compliance"];
const techTrends = ["Application Development", "Data & Analytics", "Automation", "Integration", "ABAP", "SAC", "FIORI", "RPA", "CPI", "AI"];

const ProductLifeCycle = () => {
 return (
 <SolutionPageLayout
      title="Product Lifecycle Management"
      subtitle="Unify product data and engineering change processes with SAP PLM and multi-CAD document management."
      breadcrumbs={[{ name: "Solutions" }, { name: "SAP HXM" }, { name: "Product Lifecycle", path: "/solutions/product-lifecycle" }]}
      backgroundImage="https://ik.imagekit.io/hps6th7vy/sria/Solutions/lifecycle-hero.jpg?tr=f-auto,q-auto,w-1600"
      impactHeading={<>Impact-Driven <br /><span className="text-orange-600 pt-2">SAP Growth</span></>}
      impactDescription="Accelerate innovation and streamline product development with SAP PLM. From strategy to execution, we help you manage the entire product lifecycle efficiently."
      trustedFeatures={trustedPartnerFeatures}
      endToEndServices={endToEndServices}
      techTrends={techTrends}
      ctaText="Talk to a PLM Consultant"
    >
 {/* Hero Section */}

 {/* SECTION 2: Strategy & Roadmap */}
 <section className="section-padding px-6 lg:px-12 bg-white">
 <div className="max-w-7xl mx-auto">
 <div className="mb-12">
 <h2 className="text-base font-bold text-slate-900">
 PLM Strategy & Roadmap
 </h2>
 </div>
 <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
 {strategyServices.map((card, i) => (
 <div
 key={i}
 className="group p-8 bg-slate-50 hover:bg-white border border-slate-100 hover:border-orange-100 rounded-xl shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1"
 >
 <div className="mb-6 p-3 bg-orange-50 w-fit rounded-lg group-hover:bg-orange-200 group-hover:text-white transition-colors duration-300">
 {card.icon}
 </div>
 <h3 className="text-base font-semibold mb-4 text-slate-900">
 {card.title}
 </h3>
 <ul className="space-y-2">
 {card.items.map((item, j) => (
 <li
 key={j}
 className="text-slate-600 text-sm flex items-center gap-2"
 >
 <span className="w-1.5 h-1.5 bg-orange-400 rounded-full" />{" "}
 {item}
 </li>
 ))}
 </ul>
 </div>
 ))}
 </div>
 </div>
 </section>

 {/* SECTION 3: Implementation & Integration */}
 <section className="section-padding px-6 lg:px-12 bg-slate-50">
 <div className="max-w-5xl mx-auto">
 <div className="mb-16 text-center">
 <h2 className="text-base font-bold text-slate-900 flex items-center justify-center gap-3">
 <Settings className="text-orange-600" /> Implementation & Integration
 </h2>
 </div>
 <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
 {integrationServices.map((step, i) => (
 <div key={i} className="bg-white p-6 rounded-lg shadow-sm hover:shadow-md transition-shadow duration-300 border-t-4 border-orange-500">
 <h3 className="text-base font-bold text-slate-900 mb-2">
 {step.title}
 </h3>
 <p className="text-slate-600">{step.desc}</p>
 </div>
 ))}
 </div>
 </div>
 </section>

 {/* SECTION 4: Collaboration & BOM (Split Layout) */}
 <section className="section-padding px-6 lg:px-12 bg-white">
 <div className="max-w-7xl mx-auto flex flex-col md:flex-row items-center gap-12">
 <div className="md:w-1/2">
 <h2 className="text-base font-bold text-slate-900 mb-6">
 Engineering Collaboration
 </h2>
 <div className="space-y-6">
 {collaborationAndBom.map((group, i) => (
 <div key={i}>
 <h3 className="font-semibold text-lg text-slate-800 mb-2 flex items-center gap-2">
 {group.icon}
 {group.title}
 </h3>
 <p className="text-slate-600 pl-7 border-l-2 border-orange-200 ml-2">
 {group.desc}
 </p>
 </div>
 ))}
 </div>
 </div>
 <div className="md:w-1/2 flex justify-center">
 <div className="w-64 h-64 bg-orange-100 rounded-full flex items-center justify-center relative overflow-hidden">
 <div className="absolute inset-0 bg-orange-200/50 transform -rotate-12 translate-x-10 translate-y-10"></div>
 <Share2 className="w-24 h-24 text-orange-600 relative z-10" />
 </div>
 </div>
 </div>
 </section>

 {/* SECTION 5: Compliance & Change Management */}
 <section className="section-padding px-6 lg:px-12 bg-slate-50">
 <div className="max-w-7xl mx-auto">
 <h2 className="text-base font-bold text-slate-900 mb-12 text-center">
 Document & Change Management
 </h2>
 <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
 {complianceAndChange.map((service, i) => (
 <div
 key={i}
 className="bg-white p-8 rounded-xl border border-slate-200 hover:border-orange-600 hover:ring-1 hover:ring-orange-600 transition-all duration-300 group text-center"
 >
 {service.icon}
 <h3 className="text-base font-bold mb-3">{service.title}</h3>
 <p className="text-slate-600">{service.desc}</p>
 </div>
 ))}
 </div>
 </div>
 </section>

 {/* SECTION 6: Analytics & Support (List w/ Spinner) */}
 <section className="section-padding px-6 lg:px-12 bg-white">
 <div className="max-w-6xl mx-auto">
 <div className="bg-slate-50 border-l-4 border-orange-600 rounded-r-lg shadow-lg p-8 md:p-12 flex flex-col md:flex-row items-center justify-between gap-8">
 <div className="md:w-2/3">
 <div className="flex items-center gap-3 mb-4">
 <BarChart className="text-orange-600 w-8 h-8" />
 <h2 className="text-base font-bold text-slate-900">
 Analytics & Support
 </h2>
 </div>
 <ul className="space-y-3 mb-6">
 {analyticsAndSupport.map((service, i) => (
 <li key={i} className="flex items-center gap-3 text-slate-600">
 <div className="w-1.5 h-1.5 bg-orange-500 rounded-full" />
 {service}
 </li>
 ))}
 </ul>
 </div>
 <div className="md:w-1/3 flex justify-center">
 <div className="hidden md:block w-32 h-32 border-4 border-dashed border-orange-200 rounded-full flex items-center justify-center animate-spin-slow">
 <Wrench className="w-12 h-12 text-orange-300" />
 </div>
 </div>
 </div>
 </div>
 </section>

 {/* SECTION 10: Call to Action */}
    </SolutionPageLayout>
 );
};

export default ProductLifeCycle;

