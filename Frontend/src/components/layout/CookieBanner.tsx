...
const CookieBanner = () => {
  ...
  const save = (state: ConsentState, p = prefs) => {
    ...
    setVisible(false);
    // Notifies the deferred GTM loader set up in index.html — it doesn't
    // start its load timer at all until analytics consent actually arrives
    // here, rather than merely "not yet rejected".
          ...
          <div className="bg-slate-950 border border-white/10 rounded-2xl shadow-2xl shadow-black/40 overflow-hidden">
            ...
            <div className="flex flex-col sm:flex-row sm:items-center gap-3 px-4 py-3">
              ...
              <div className="flex items-center gap-2 flex-wrap sm:flex-nowrap sm:flex-shrink-0 sm:ml-auto">
                ...
                <button
                  onClick={acceptAll}
                  className="px-3 py-1.5 rounded-lg bg-orange-500 hover:bg-orange-600 text-slate-900 text-xs font-bold transition-colors"
                >
                  Accept All
                </button>
